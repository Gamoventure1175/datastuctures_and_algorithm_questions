...
def twoSum(nums: list[int], target: int):
    """
    Finds the indices i and j such that nums[i] + nums[j] == target and i != j.
    ...
    Return:
        - list[int]: pair of indices i and j

    Note: Assumption that every input has exactly one pair of
    indices i and j that satisfy the condition.
    """

    indices_tracker = {nums[i]: i for i in range(len(nums))}

    for i in range(len(nums)):
        complement = target - nums[i]
        possible_index = indices_tracker.get(complement, None)

        if possible_index != None and i != possible_index:
