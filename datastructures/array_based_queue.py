...
# Queue.enqueue() -> add element to end of the queue
# Queue.dequeue() -> remove element from the start of the queue
...
# Making a python circular array based queue
...
class CircularArrayQueue:
    ...
    def dequeue(self):
        ...
        if (
            ...
            and len(self._data) > CircularArrayQueue.DEFAULT_CAPACITY
        ):
...
if __name__ == "__main__":
    example = CircularArrayQueue()
    for number in range(1, 10):
        example.enqueue(number)
        print(
            f"Current queue: {example}. Current size of the queue: {len(example)}. Current front: {example.first()} \n"
        )
    ...
    print(f"Current first: {example.first()}")
